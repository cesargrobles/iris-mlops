...
def run_inference(model, X_infer: pd.DataFrame, *, include_proba: bool = False) -> pd.DataFrame:
    ...
    # 4) Optionally attach probabilities (Critical for risk-scoring models)
    if include_proba:
        ...
        if proba.shape[1] < 2:
            raise ValueError(
                f"Fatal: predict_proba must return at least 2 columns for classification. Got {proba.shape}")

        if proba.shape[1] == 2:
            # Binary: attach positive-class probability
            df_pred["proba"] = proba[:, 1].astype(float)
        else:
            # Multiclass: attach max probability as confidence score
            df_pred["proba"] = np.max(proba, axis=1).astype(float)
